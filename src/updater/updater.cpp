                        ...
                        const SegmentLookupTable &segment_speed_lookup)
{
    ...
    tbb::parallel_for(range, [&](const auto &range) {
        ...
        for (auto geometry_id = range.begin(); geometry_id < range.end(); geometry_id++)
        {
            ...
            auto rev_weights_range = segment_data.GetForwardWeights(geometry_id);
            auto rev_durations_range = segment_data.GetForwardDurations(geometry_id);
            auto rev_datasources_range = segment_data.GetForwardDatasources(geometry_id);
